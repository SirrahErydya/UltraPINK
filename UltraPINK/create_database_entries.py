...
from pinkproject.models import upload_to_dataset_folder
from som.models import upload_to_som_folder
from som import som
...
def create_dataset_models(dataset_name, descr, data_file, project, cat_file=None):
    dataset = pmodels.Dataset(
        project=project,
        dataset_name=dataset_name,
        description=descr,
        length = len(som.get_data(data_file))
    )
    path = upload_to_dataset_folder(dataset)
    os.makedirs(path, exist_ok=True)
    dataset.data_path = data_file
    dataset.catalog_path = cat_file
    dataset.save()
    return dataset
...
def save_prototype_grid(som_model, path):
    ...
    if som_model.layout == 'cartesian-2d':
        ...
        for prototype in prototypes:
            ...
            axis.imshow(Image.open(urlopen(prototype.image)), cmap='gray')
            i += 1
...
def save_proto_img(np_img, path, layout='cartesian-2d'):
